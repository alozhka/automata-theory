...
public class GrammarTests
{
    ...
    public static TheoryData<string> ListInvalidStatements()
    {
        return new TheoryData<string>
        {
            ...
            "dayzint iffy;",
            // 11. Отсутствие типа возвращаемого значения при наличии returnal
            "funkotron test() { returnal 5; }",
            // 12. Неправильный синтаксис цикла for - отсутствует точка с запятой
            "forza (dayzint i = 0 i < 5; i = i + 1) { }",
            // 13. Пустое выражение в условии
            "iffy () { }",
            // 14. Неправильный синтаксис вызова функции
            "exodus();",
            // 15. Использование несуществующего оператора
            "dayzint x = y ** z;",
            // 16. Неправильный синтаксис nullable типа
            "? dayzint x;",
            // 17. Двойное отрицание без скобок (может быть валидным, но проверим)
            "statum x = !!y;",
            // 18. Неправильное завершение комментария
            "\\* комментарий без закрытия\ndayzint x;",
            // 19. Breakout вне цикла (синтаксически может быть валидным, но логически нет)
            "breakout;",
            // 20. Присваивание без переменной
            "= 5;",
        };
    }
}